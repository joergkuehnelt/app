...
/**
 * WooCommerce B2B Purchase order Gateway
 *
 * @version 3.3.9
 */
...
class WCB2B_Gateway_PurchaseOrder extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->id                 = 'wcb2b_purchaseorder_gateway';
        $this->icon               = apply_filters( 'wcb2b_purchaseorder_gateway_icon', '' );
        $this->has_fields         = true;
        $this->method_title       = __( 'Purchase order', 'woocommerce-b2b' );
        $this->method_description = __( 'Allows purchase order payments.', 'woocommerce-b2b' );
    ...
    public function init_form_fields() {
        $this->form_fields = apply_filters( 'wc_offline_form_fields', array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable purchase order payments', 'woocommerce-b2b' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'Purchase order', 'woocommerce-b2b' ),
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => __( 'Please add your PO number', 'woocommerce-b2b' ),
                'desc_tip'    => true,
            ),
            'instructions' => array(
    ...
    public function process_payment( $order_id ) {
        ...
        if ( isset( $_POST['wcb2b_purchaseorder_number'] ) && trim( $_POST['wcb2b_purchaseorder_number'] ) != '' ) {
            $order->update_meta_data( '_wcb2b_purchaseorder_number', sanitize_text_field( $_POST['wcb2b_purchaseorder_number'] ) );
            $order->save();
        }
    ...
    public function payment_fields() {
        woocommerce_form_field( 'wcb2b_purchaseorder_number', array(
            'type'          => 'text',
            'class'         => array( 'form-row-wide' ),
            'label'         => esc_html__( 'Purchase order number', 'woocommerce-b2b' ),
            'required'      => true
        ) );
    }
    ...
    public function validate_fields() {
        ...
        if ( ! $_POST['wcb2b_purchaseorder_number'] ) {
            wc_add_notice( __( 'Please enter a purchase order number.', 'woocommerce-b2b' ), 'error' );
        }
        return true;
    }
